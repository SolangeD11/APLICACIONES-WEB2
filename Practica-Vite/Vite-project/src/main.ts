...
(async ()=>{
  ...
  data.forEach((idioma: Iidioma) => {
    divTable += `<tr><td>${idioma.id}</td><td>${idioma.nombreidioma}</td><td><button class="btn btn-delete">Eliminar</button></td><td><button class="btn btn-update">Actualizar</button> </td> </tr>`
  })
  divTable += `</table>`
  ...
  btnAgregar?.addEventListener('click', ()=>{
    ...
    btnSave?.addEventListener('click', async (e)=>{
      e.preventDefault()
      const nombreidioma = document.querySelector<HTMLInputElement>('#nombreidioma')!.value
      const response = await fetch('http://localhost:3000/api/idioma', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
        },
        body: JSON.stringify({nombreidioma, origenId:1})
      })
      const data = await response.json()
      console.log(data)
      // reload page
  ...
  document.querySelectorAll<HTMLButtonElement>('.btn-update').forEach(btn=>{
    btn.addEventListener('click', async ()=>{ 
      ...
      <div class="mb-3">
        <label for="code" class="form-label">Code</label>
        <input type="text" class="form-control" id="code" aria-describedby="code" value="${data.nombreidioma}">
      </div>
      <div class="mb-3">
        <label for="detail" class="form-label">Detail</label>
        <input type="text" class="form-control" id="detail" value="${data.origenId}">
      </div>
      <button type='submit'  class="btn btn-save">Save</button>
      ${btnCancel}
      </form>`
